...
> **TypeScript warning:** Some libraries (notably `lucide-react`) don't ship `.d.ts` files for their deep import paths. Prefer `optimizePackageImports` when available.
...
Build tools work best when import and file-system paths are obvious at build time.
...
RSC→client serialization deduplicates by object reference, not value. Do transformations (`.toSorted()`, `.filter()`, `.map()`) in client, not server.
...
Avoid using mutable module-level variables to share request-scoped data in RSC/SSR.
...
When loading static assets in route handlers or server functions, hoist the I/O operation to module level. Module-level code runs once when the module is first imported.
...
export async function GET(request: Request) {
  ...
  return new ImageResponse(/* ... */, { fonts: [{ name: 'Inter', data: fontData }] })
}
```
...
export async function GET(request: Request) {
  ...
  return new ImageResponse(/* ... */, { fonts: [{ name: 'Inter', data: font }] })
}
```
...
**Avoid inline objects as arguments** — `React.cache()` uses shallow equality (`Object.is`) to determine cache hits.
...
**Don't use passive when:** implementing custom swipe gestures, custom zoom controls.
...
**Always wrap in try-catch:** `getItem()` and `setItem()` throw in incognito/private browsing (Safari, Firefox), when quota is exceeded, or when disabled.
...
When a memoized component has a default value for a non-primitive optional parameter (array, function, or object), extract it to a constant.
...
If a side effect is triggered by a specific user action, run it in that event handler — not in an effect.
...
Use lazy initialization when computing initial values from localStorage/sessionStorage, building data structures, reading from the DOM, or performing heavy transformations.
...
When a value changes frequently and you don't want a re-render on every update, store it in `useRef` instead of `useState`. Updating a ref does not trigger a re-render.
...
When rendering content that depends on client-side storage (localStorage, cookies), inject a synchronous script that updates the DOM before React hydrates.
...
For *expected* mismatches (random IDs, dates, locale/timezone formatting), wrap the dynamic text in an element with `suppressHydrationWarning`. Do not use this to hide real bugs.
...
Use explicit ternary operators (`? :`) instead of `&&` for conditional rendering when the condition can be `0`, `NaN`, or other falsy values.
...
Avoid interleaving style writes with layout reads. Batch all writes together, then read.
...
Chaining `.map().filter(Boolean)` creates an intermediate array and iterates twice.
...
Finding the smallest or largest element only requires a single pass through the array.
...
Effect Event functions do not have a stable identity. Do not include the function returned by `useEffectEvent` in a `useEffect` dependency array.
...
function ChatRoom({ roomId, onConnected }: { roomId: string; onConnected: () => void }) {
  ...
  }, [roomId, handleConnected])  // ❌ handleConnected in deps
}
```
...
Do not put app-wide initialization that must run once per app load inside `useEffect([])`. Use a module-level guard or top-level init in the entry module instead.
...
**Correct: stable subscription using useEffectEvent**
...
function SearchInput({ onSearch }: { onSearch: (q: string) => void }) {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timeout)
  }, [query])  // stable — no onSearch dependency
}
```
